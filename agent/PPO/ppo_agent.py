...
log_file = open('loss.csv', mode='w+')
print('v_loss, pg_loss', file=log_file)
...
class PPOAgent:
    ...
    def compute_discounted_return(self, rewards: np.ndarray, last_value: np.ndarray) -> np.ndarray:
        """
        ...
        # NOTE: G_{n-1} = r_{n-1} + gamma * last_value
        # NOTE: G_t = r_t + gamma * G_{t+1}
    ...
    def compute_gae(self, rewards: np.ndarray, values: np.ndarray, last_value: np.ndarray) -> np.ndarray:
        """
        ...
        # NOTE: delta_t = r_t + gamma * (V(s_{t+1}) - V(s_t))
        # NOTE: adv_t = delta_t + gamma * lamb * adv_{t+1}
        for t in reversed(range(n_step)):
            if t == n_step - 1:
    ...
    def get_action(self, obs: dict) -> tuple | dict[str:float, str:float]:
        ...
        with torch.no_grad():
            if self.is_training:
                ...
                return {"motor": np.clip(action[0], -1, 1), 'steering': np.clip(action[1], -1, 1)}
    ...
    def learn(self) -> None:
        ...
        for i in range(self.sample_n_epoch):
            ...
            for j in range(sample_n_mb):
                ...
                print(f'{v_loss}, {pg_loss}', file=log_file)

        self.memory_counter = 0

    def store_trajectory(self, obs: dict, action: np.ndarray, value: np.ndarray, a_logp: np.ndarray, reward: float) -> None:
        """
    ...
    def load_model(self) -> None:
        ...
        self.policy_net.load_state_dict(torch.load(self.model_path, map_location=loc))
